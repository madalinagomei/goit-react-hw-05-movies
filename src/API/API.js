...
export const fetchTreding = async () => {
    const rest = await axios.get('treding/all/day?api_key=${API_KEY}')
    return rest.data;
};
...
export const fetchSearchMovies = async query => {
    const res = await axios.get('search/movie?api_key=${API_KEY}&language=en-US&page=1&include+adult=false&query=${query}');
    return res.data;
};
